...
class loggedInUser {
    ...
    public function resetTokenExpirationDateForUser($username){
        global $mysqli,$db_table_prefix;
        $stmt = $mysqli->prepare("UPDATE ".$db_table_prefix."registered_devices
			SET
    ...
    //Update a user rsaPublicKey
    public function updateRsaPublicKey($rsaPublicKey)
    {
        global $mysqli,$db_table_prefix;
        ...
        $stmt = $mysqli->prepare("UPDATE ".$db_table_prefix."users
			...
			WHERE
			id = ?");
        $stmt->bind_param("si", $rsaPublicKey, $this->user_id);
        $stmt->execute();
        $stmt->close();
    }
    ...
    public function updateAlias($alias)
    {
        ...
        $stmt = $mysqli->prepare("UPDATE ".$db_table_prefix."users
			...
			WHERE
			id = ?");
        $stmt->bind_param("si", $alias, $this->user_id);
        $stmt->execute();
        $stmt->close();
    }
