...
function generateHash($plainText, $salt = null)
{
    if ($salt === null)
    {
        $salt = substr(hash('sha256', uniqid(rand(), true)), 0, 25);
    }
    else
    {
...
function fetchAllDevices($username){
    global $mysqli,$db_table_prefix;
    $stmt = $mysqli->prepare("SELECT `deviceIdentifier`, `ipAddress`, `lastHeartbeatDate`, `tokenExpirationDate` FROM registered_devices WHERE username LIKE ?");
    $searchterm = $username."@%";
	$stmt->bind_param("s", $searchterm);
    $stmt->execute();
    $stmt->bind_result($deviceIdentifier, $ipAddress, $lastHeartbeatDate, $tokenExpirationDate);

    $row[] = array();
    while ($stmt->fetch()){
        $row[] = array('deviceIdentifier' => $deviceIdentifier, 'ipAddress' => $ipAddress, 'lastHeartbeatDate' => $lastHeartbeatDate, 'tokenExpirationDate' => $tokenExpirationDate);
    }
    $stmt->close();
    return ($row);
}

function deleteDevice($deviceIdentifier, $username){
    global $mysqli,$db_table_prefix;
    $stmt = $mysqli->prepare("DELETE FROM ".$db_table_prefix."registered_devices WHERE deviceIdentifier = ? AND username LIKE ?");
	$searchterm = $username."@%";
    $stmt->bind_param("ss", $deviceIdentifier, $searchterm);
    $stmt->execute();
    $stmt->close();
}
...
function fetchAllUsers()
{
	...
	$stmt = $mysqli->prepare("SELECT 
		...
		FROM ".$db_table_prefix."users");
	$stmt->execute();
	$stmt->bind_result($id, $user, $display, $password, $email, $token, $activationRequest, $passwordRequest, $active, $title, $signUp, $signIn, $rsaPublicKey);
	
	while ($stmt->fetch()){
		$row[] = array('id' => $id, 'username' => $user, 'alias' => $display, 'password' => $password, 'email' => $email, 'activation_token' => $token, 'last_activation_request' => $activationRequest, 'lost_password_request' => $passwordRequest, 'active' => $active, 'title' => $title, 'sign_up_stamp' => $signUp, 'last_sign_in_stamp' => $signIn, 'rsaPublicKey' => $rsaPublicKey);
	}
	$stmt->close();
	return ($row);
}
...
function updatealias($id, $display)
{
	global $mysqli,$db_table_prefix;
	$stmt = $mysqli->prepare("UPDATE ".$db_table_prefix."users
		SET alias = ?
