...
{	
	...
	{
		//Update display name
		if ($userdetails['alias'] != $_POST['display']){
			$alias = trim($_POST['display']);

			if(minMaxRange(5,25,$alias))
			{
				$errors[] = lang("ACCOUNT_DISPLAY_CHAR_LIMIT",array(5,25));
			} else {
				if (updatealias($userId, $alias)){
					$successes[] = lang("ACCOUNT_alias_UPDATED", array($alias));
				}
				else {
					$errors[] = lang("SQL_ERROR");
				}
		...
		if(isset($_POST['activate']) && $_POST['activate'] == "activate"){
			if (setUserActive($userdetails['activation_token'])){
				$successes[] = lang("ACCOUNT_MANUALLY_ACTIVATED", array($alias));
			}
			else {
				$errors[] = lang("SQL_ERROR");
			}
		...
		if ($userdetails['title'] != $_POST['title']){
			...
			if(minMaxRange(1,50,$title))
			{
				...
				if (updateTitle($userId, $title)){
					$successes[] = lang("ACCOUNT_TITLE_UPDATED", array ($alias, $title));
				}
				else {
					$errors[] = lang("SQL_ERROR");
				}
...
<input type='text' name='display' value='".$userdetails['alias']."' />
</p>
<p>
<label>Email:</label>
